...
export default function ModalUpdateBooking (): JSX.Element {
  const [loading, setloading] = useState(false);
  const closeModal = bookingModalZustand(state => state.closeModalUpdateBooking)
  const booking = bookingModalZustand(state => state.editBooking.booking)
  ...
  const handlerSubmit = async (event: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    setloading(true);
    console.log(booking)
    const response  = await bookingsUseCases.updateBooking({id_booking:booking.id_booking, id_room: booking.id_room, entry_date: startDate, exit_date: endDate});
    if (response) {
      setloading(false);
      setEndDate("")
      setStartDate("")
  ...
  return (
    <section className={sectionMainCss}>
      ...
      <form
        ...
        <h1 className={h1Css}>Actualizar reserva</h1>
